...
class LocationWebSocket(
    private val onLocation: (LocationMsg) -> Unit,
    private val onEvent: ((LocationMsg) -> Unit)? = null
) {
        ...
        webSocket = client.newWebSocket(request, object : WebSocketListener() {
            ...
            override fun onMessage(webSocket: WebSocket, text: String) {
                try {
                    ...
                    when (msg.type) {
                        "location" -> onLocation(msg)
                        "permission_granted", "permission_denied" -> onEvent?.invoke(msg)
                        else -> {}
                    }
                } catch (_: Exception) {
                }
